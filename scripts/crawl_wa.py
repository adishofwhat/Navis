...
OUTPUT_DIR = Path("crawl4ai_docs/whatsapp")
...
LINK_PATTERN = re.compile(r'\[(?:[^\]]+)\]\((https://faq\.whatsapp\.com/[^)]+)\)')
    ...
    async with AsyncWebCrawler() as crawler:
        while urls and article_count < MAX_TOTAL_ARTICLES:
            ...
            for link in found_links:
                if link.startswith("https://faq.whatsapp.com/") and link not in seen_urls:
                    urls.append(link)
